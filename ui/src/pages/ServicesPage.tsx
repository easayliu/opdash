...
export function ErrorRate({ rate }: { rate: number }) {
  ...
  return (
    <span className={cn('inline-flex items-center justify-end gap-1 tabular-nums', tone)}>
      {pct >= 1 && <AlertTriangleIcon className="size-3" aria-label={pct >= 5 ? '错误率高' : '错误率偏高'} />}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex items-center gap-2 border-b border-border bg-card px-3 py-1.5 text-xs text-muted-fg">
        <span>只统计入口 span（Server / Consumer）；点一行看接口和趋势。</span>
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-card">
        ...
        {rows.length > 0 && (
          <table className={cn('w-full border-collapse text-xs', q.isFetching && 'opacity-70')}>
              ...
              <tr className="border-b border-border">
                {COLUMNS.map((c) => (
                  <th
                    key={c.key}
                    title={c.title}
                    className={cn('cursor-pointer px-3 py-1.5 font-medium select-none hover:text-fg', c.align === 'right' ? 'text-right' : 'text-left', sort.key === c.key && 'text-accent')}
            ...
            <tbody>
              {rows.map((s) => (
                <tr key={s.service} className="row-hover cursor-pointer border-b border-border/60" onClick={() => navigate(`/services/${encodeURIComponent(s.service)}`)}>
                  <td className="px-3 py-1.5 font-medium">{s.service || '(空)'}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatNumber(s.requests)}</td>
                  <td className="px-3 py-1.5 text-right text-muted-fg tabular-nums">{s.rps < 10 ? s.rps.toFixed(2) : Math.round(s.rps)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{s.errors ? formatNumber(s.errors) : <span className="text-muted-fg">0</span>}</td>
                  <td className="px-3 py-1.5 text-right">
                    <ErrorRate rate={s.error_rate} />
                  </td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatDurationMs(s.p50_ms)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatDurationMs(s.p95_ms)}</td>
                  <td className="px-3 py-1.5 text-right font-medium tabular-nums">{formatDurationMs(s.p99_ms)}</td>
                  <td className="px-3 py-1.5 text-right text-muted-fg tabular-nums">{formatDurationMs(s.max_ms)}</td>
                </tr>
              ))}
            </tbody>
          </table>
