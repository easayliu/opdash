    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-border bg-card px-3 py-2.5 md:px-4 md:py-3">
        ...
        <span className="-mx-3 flex w-[calc(100%+1.5rem)] items-center gap-2 overflow-x-auto px-3 [scrollbar-width:none] md:mx-0 md:ml-auto md:w-auto md:flex-wrap md:overflow-visible md:px-0 [&>*]:shrink-0">
          <Hint text="图表中的变化均与此时段对比" asChild>
            <Select
              value={compare}
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3 md:p-4">
          ...
          <Card
            ...
            title={
              <span className="flex flex-wrap items-baseline gap-x-2">
                与{cmpShort}相比发生变化的{kind === 'entry' ? '接口' : '下游调用'}
                {movers.length > 0 && (
                  <span className="text-2xs font-normal text-muted-fg">
                    {movers.length} 个<span className="hidden sm:inline"> · 按影响面排序，而非按百分比</span>
                  </span>
                )}
              </span>
            }
            extra={
              movers.length > 6 && (
                <Button size="xs" variant="ghost" onClick={() => setAllMovers((v) => !v)}>
                  {allMovers ? '仅显示前 6 个' : `全部 ${movers.length} 个`}
                </Button>
              )
            }
          >
            {movers.length === 0 ? (
              <div className="px-3 py-3 text-2xs text-muted-fg">
                没有接口发生明显变化：错误未增加，P95 也未上升（两个时段均满 100 次请求才参与比较，请求数不足时个别慢请求只是噪声）。
              </div>
            ) : (
              <ul>
        ...
        <Card
          ...
          extra={
            <>
              {ops.data?.truncated && (
                <Hint text="本服务的接口过多（常见于将 SQL 或 ID 拼入 span 名），仅统计了请求量最大的部分">
...
function OpTag({ o }: { o: OperationStat }) {
  if (o.requests === 0 && o.prev) return <Badge tone="warn">已消失</Badge>
...
function MoverRow({ m, selected, onSelect }: { m: Mover; selected: boolean; onSelect: () => void }) {
  return (
    <div>
      <Hint text={selected ? '再次点击以查看整个服务' : '只看这个接口的趋势'} asChild>
        <button
          type="button"
          onClick={onSelect}
