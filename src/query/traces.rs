...
impl TraceQueries<'_> {
    ...
    /// 不像检索那样只取前 N 条。
    ///
    /// **整窗聚合，但不贵**——检索那边靠 [`PROBE_WINDOWS_MS`] 把窗口缩到 1 分钟，这条缩不了
    /// （画的就是全貌），看着像这一页的大头，实测不是（线上 2026-09-17，入口 span、不选服务）：
    ///
    /// | 窗口 | 读行数 | 读量 | 耗时 |
    /// |---|---|---|---|
    /// | 1 小时 | 2514 万 | 139.4 MB | 388 ms |
    /// | 6 小时（不选服务时的上限） | 1.07 亿 | 396.8 MB | 450 ms ~ 2.0 s |
    /// | 同窗口的检索探测（对比） | 72 万 | 28.3 MB | 212 ms |
    ///
    /// 拆开看：去掉 `countIf(status_code = 'Error')` 省 6.5 MB / 75 ms，连 `duration_ns` 都不读
    /// （只按桶 `count()`）才降到 78.7 MB / 211 ms。也就是说读的这几列都很窄，墙钟时间的大头是
    /// 分布式扇出和找 granule 的固定开销，不是扫描量——1 小时和 6 小时差 4 倍的读量，耗时却在
    /// 同一个量级。
    ///
    /// **所以别为了「错开去蹭 query condition cache」把它串到检索后面**：试过，没用。热力图默认
    /// 带 `span_kind IN ('Server', 'Consumer')`（见 `api::traces::build_filter` 的
    /// `entry_by_default`），按时间排的检索却不带；两条的时间窗也不一样（整窗 vs 尾部 1 分钟）。
    /// 条件对不上就不共享：实测探测跑完紧接着的热力图读 1845 万行，和没跑过探测的冷跑 1990 万行
    /// 基本一样（而同一条热力图跑两遍是 2132 万 → 385 万行，缓存本身是有效的）。串行化只是让
    /// 整页画完晚 200 ms 以上，白亏。
    pub fn heatmap(
        &self,
        filter: &TraceFilter,
